        ...
        child: Column(
          ...
          children: [
            ...
            Expanded(
              child: ListView(
                children: [
                  for (var alertItem in _alertData)
                    Column(
                      children: [
                        // Dismissible : child로 설정된 위젯을 좌우로 스크롤 할수 있음
                        // 스크롤로 없애도 위젯을 여전히 남아있기 때문에 없애주는 작업이 따로 필요함
                        Dismissible(
                          key: Key(alertItem["title"]),
                          onDismissed: (direction) => _onDismissed(alertItem),
                          // 오른쪽으로 스크롤
